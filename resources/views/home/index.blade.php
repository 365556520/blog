...
<div class="banner">
  <section class="box">
    ...
    <div class="avatar"><a href="#"><span>超级公司</span></a> </div>
  </section>
</div>
<div class="template">
  <div class="box">
    ...
    <ul>
        @foreach($pics as $p)<li><a href="{{url('a/'.$p->art_id)}}"  target="_blank"><img src="{{url($p->art_thumb)}}"></a><span>{{$p->art_title}}</span></li>@endforeach
    </ul>
  </div>
</div>
<article>
  ...
  <div class="bloglist left">
    ...
    <p class="dateview"><span>&nbsp;{{date('Y年m月d日',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span></p>
    @endforeach
      <div class="page">
            {{$data->links()}}
      </div>
